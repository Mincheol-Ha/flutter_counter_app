...
void main() {
  ...
  // MyApp은 const로 선언했으므로 불변 객체임을 의미 - 성능 최적화 목적
}
...
class MyApp extends StatelessWidget {
  ...
  // 실행하는 애브이 종류에 따라 상태가 그대로 유지되기로 함 (android vs chrome)
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _incrementCounter() {
    ...
    setState(() {
      ...
      // 이를 통해 변경됨 state를 UI에 반영하기 위해 build 메서드 재실행함
  ...
  Widget build(BuildContext context) {
    ...
    // 위듸 예에서는 _incrementCounter가 호출되어 상태 변화가 생겼을때 build 재실행
...
/*     Scaffold
      ...
      appBar: AppBar(
        ...
        // MyHomePage 객체에 정의된 title 값을 가져와 AppBar 제목을 설정함
        title: Text(widget.title),
      ),
      body: Center(
        // Center는 레이어웃 위젯 - 자식요소를 받아 중앙에 배치
        child: Column(
          // Cloumn도 레이아웃 위젯 - 자식요소를 받아 수직으로 배치
